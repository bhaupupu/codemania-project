...
import { generateAdaptiveTasksForGameSync } from './adaptiveChallenge.service';
import {
...
export async function createRoom(
  ...
  try {
    ...
    logger.warn(`[createRoom] Could not persist game to MongoDB (in-memory mode active): ${err}`);
  }
...
export function startGame(roomCode: string): IGame | null {
  ...
  const tasks = generateAdaptiveTasksForGameSync(
    game.language,
    game.skillLevel,
    coders.map((c) => ({ userId: c.userId, displayName: c.displayName })),
    tasksPerPlayer
  );
