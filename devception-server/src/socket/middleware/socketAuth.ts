...
async function resolveProfile(
  ...
  // Cache miss — attempt to upsert and fetch from MongoDB
  let userDisplayName = decoded.name ?? 'Player';
  let userAvatarUrl = decoded.picture ?? '';
  ...
  try {
    ...
    logger.warn(`[socketAuth] DB lookup failed, falling back to token metadata: ${err}`);
  }
