...
import { recordTaskAttempt } from '../../services/learningProfile.service';
...
export function registerTaskHandlers(io: Server, socket: AuthenticatedSocket): void {
  socket.on(
    ...
    async ({
      ...
      // AI Player Learning System: Track performance signals, mistakes, and update player profile
      recordTaskAttempt(
        socket.userId,
        task,
        submittedCode,
        ...
        logger.debug(`[AI Learning] recordTaskAttempt error: ${err.message}`);
      });
